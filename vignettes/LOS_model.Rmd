...
This vignette details why the `LOS_model` dataset was created, how to load and use it in learning/teaching regression modelling using Generalized Linear Models.  The data are 10 sets of 30 simulated patient records, representing 10 different hospitals ("Trusts").  The dataset contains:
...
Now lets look at the distributions of ages and LOS.  Age is rather odd.  Could be uniform, could be bimodal (or more), but nothing clear here.  LOS is highly skewed, and this is not a surprise for count data, as it can only be >0, in integer values, and most patients don't stay very long, but a few do.
...
<<<<<<< HEAD
Let's look at LOS first.  It is a count, and therefore has a boundary at zero (you can't have counts smaller than zero), and is highly right-skewed, as most patietns are not in longbut a few are.
We can use the 'Poisson' distrubtion for this.
...
### Modelling death:
>>>>>>> bd138ffacf58d4325708ce838a2dd8957ea01ffb
